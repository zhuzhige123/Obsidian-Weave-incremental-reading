...
/** 同一 topicId 对应多个可见 .irdeck 文件时的冲突摘要。 */
export interface IRPointFileTopicConflict {
	topicId: string;
	topicName: string;
	paths: string[];
}
...
/** 库内可见路径扫描并刷新专题索引后的统计。 */
export interface IRPointFilesIndexRefreshResult {
	/** 扫描到的 .irdeck 文件数（可见库内路径） */
	scanned: number;
	/** 去重后的专题 ID 数量 */
	topicCount: number;
	/** 存在多文件同 topicId 的专题组数 */
	duplicateTopicGroups: number;
	added: number;
	updated: number;
	removed: number;
	conflicts: IRPointFileTopicConflict[];
}
