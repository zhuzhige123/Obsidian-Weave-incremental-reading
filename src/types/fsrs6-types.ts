...
import type { FSRSCard, FSRSParameters } from "../data/types";
...
export interface FSRS6Parameters extends FSRSParameters {
	...
	w: [
		...
		number,
		number,
		number, // w0-w4: 初始稳定性参数
		number,
		number, // w5-w6: 难度调整参数
		number,
		number,
		number, // w7-w9: 回忆稳定性参数
		number,
		number,
		number,
		number, // w10-w13: 遗忘稳定性参数
		number,
		number,
		number, // w14-w16: 评分调整参数
		number,
		number, // w17-w18: 短期记忆参数 (FSRS6新增)
		number,
		number // w19-w20: 长期稳定性参数 (FSRS6新增)
	];
