...
const isIrStandalone = typeof __WEAVE_IR_STANDALONE__ !== 'undefined' && __WEAVE_IR_STANDALONE__;

/** 独立 IR 插件对外暴露的高级功能 ID（仅增量阅读相关）。 */
export const PREMIUM_FEATURES = isIrStandalone ? IR_PREMIUM_FEATURES : IR_PREMIUM_FEATURES;

export const FEATURE_METADATA = isIrStandalone ? IR_FEATURE_METADATA : IR_FEATURE_METADATA;

export const PREMIUM_BENEFIT_FEATURE_ORDER = isIrStandalone
	? IR_PREMIUM_BENEFIT_FEATURE_ORDER
	: IR_PREMIUM_BENEFIT_FEATURE_ORDER;

const PREMIUM_ONLY_FEATURE_IDS = isIrStandalone ? IR_PREMIUM_ONLY_FEATURE_IDS : IR_PREMIUM_ONLY_FEATURE_IDS;
...
export class PremiumFeatureGuard {
  ...
  getAnyFeatureEntryTitle(
    ...
    return this.canUseAnyFeature(featureIds, context) ? baseTitle : `${baseTitle} 🔒`;
  }
  ...
  getFeatureEntryTitle(
    ...
    return this.canUseFeature(featureId, context) ? baseTitle : `${baseTitle} 🔒`;
  }
...
const LIMITED_TIME_FEATURE_ACCESS: Partial<Record<string, LimitedTimeFeatureRule>> = isIrStandalone
	? {}
	: {};
