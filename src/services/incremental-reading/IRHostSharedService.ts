...
export interface IREnsureExternalDocumentChunkScheduledOptions {
	...
	resumeLink?: string;
	webUrl?: string;
	/** 网页选区摘录，写入 chunk.meta.notes */
	webSelectionExcerpt?: string;
}
...
export class IRHostSharedService {
	...
	private irV4SchedulerService: IRV4SchedulerService | null = null;
	...
	private async ensureIRV4SchedulerServiceReady(): Promise<IRV4SchedulerService> {
		if (!this.irV4SchedulerService) {
			this.irV4SchedulerService = new IRV4SchedulerService(this.app);
		}
		await this.irV4SchedulerService.initialize();
		return this.irV4SchedulerService;
	}
	...
	private async resolveMatchingEpubTask(
		...
		const service = await this.ensureIREpubBookmarkTaskServiceReady();
		const tasks = await service.getTasksByEpub(normalizedFilePath);
		if (tasks.length === 0) {
			return null;
		}
		...
		const deckId = String(getTaskTopicId(task) || task.deckId || "").trim();
		if (!deckId) {
			return null;
		}
