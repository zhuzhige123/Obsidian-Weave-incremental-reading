...
function createMockApp(options?: {
	...
	blocks?: Record<string, Record<string, { position: { start: { line: number }; end: { line: number } } }>>;
}) {
	...
	return {
		...
		vault: {
			getAbstractFileByPath: (filePath: string) => (files[filePath] ? { path: filePath } : null),
		},
	} as never;
}
...
describe("IRReadingTargetParser", () => {
	it("parses http urls", () => {
		const parsed = parseReadingTargetInput(createMockApp(), "https://example.com/article");
	...
	it("parses vault block references", () => {
		...
		const parsed = parseReadingTargetInput(app, "![[Notes/demo.md#^weea92dv|记忆机制]]");
	...
	it("parses heading links", () => {
		const app = createMockApp({ files: { "Notes/demo.md": true } });
		const parsed = parseReadingTargetInput(app, "[[Notes/demo.md#章节一|章节一]]");
	...
	it("parses weave-epub-reader markdown links", () => {
		...
		const raw =
			"[遥远的向日葵地 · 十九 天](obsidian://weave-epub-reader?file=" +
			encodeURIComponent(epubPath) +
