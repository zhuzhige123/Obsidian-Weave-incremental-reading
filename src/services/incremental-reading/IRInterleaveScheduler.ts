...
export class IRInterleaveScheduler {
	 ...
	 * @returns 重排后的队列
	 */
	applyInterleaving(blocks: IRBlock[], isNewSession = true): IRBlock[] {
		if (blocks.length <= 1) {
			return blocks;
		}
	...
	private interleaveByFile(blocks: IRBlock[]): IRBlock[] {
		...
		// 转换为数组并交错
		const groups: BlockGroup[] = Array.from(fileGroups.entries()).map(([key, blocks]) => ({
			key,
			blocks,
		}));
	...
	private groupByTopic(blocks: IRBlock[]): BlockGroup[] {
		...
		for (const block of blocks) {
			...
			const topic = block.tags.length > 0 ? block.tags[0] : `file:${block.filePath}`;
