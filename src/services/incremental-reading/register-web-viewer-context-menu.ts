...
export function registerWebViewerContextMenuPatch(
	...
	const tryPatchWebViewerViews = (): void => {
		...
		for (const leaf of leaves) {
			...
			proto.contextMenuItemsForSelection =
				function patchedContextMenuItemsForSelection(
					...
					if (
						!selectedText ||
						!host.shouldShowPremiumEntry(PREMIUM_FEATURES.INCREMENTAL_READING)
					) {
						return items;
					}
			...
			logger.info(
				"[Standalone IR] 已为官方 Web Viewer 选区右键菜单注入「添加到增量阅读」",
			);
		}
	};
