...
describe("IRCognitiveProfileService", () => {
	...
	test("负载与波动惩罚升高时，显式目标函数总分会下降", () => {
		...
		expect(stressed.penalties.volatility).toBeGreaterThan(
			relaxed.penalties.volatility,
		);
	});
	...
	test("记忆卡优先级信号会提升重要性分数", () => {
		...
		expect(withSignal.importanceScore).toBeGreaterThan(
			withoutSignal.importanceScore,
		);
	});
});
