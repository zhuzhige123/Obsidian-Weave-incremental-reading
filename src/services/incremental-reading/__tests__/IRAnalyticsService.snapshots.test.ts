...
describe("IRAnalyticsService snapshots", () => {
	...
	it("keeps load forecast snapshot consistent with the overall forecast projection", async () => {
		...
		const overallSnapshot = await service.getSnapshot({
			mode: "overall",
			days: 2,
		});
	...
	it("exposes translated timing bucket labels in the overall analytics snapshot", async () => {
		...
		expect(
			snapshot.timingBuckets.some((bucket) => bucket.label === "今日到期"),
		).toBe(true);
		expect(
			snapshot.timingBuckets.some((bucket) => bucket.label === "Due today"),
		).toBe(false);
	});
});
