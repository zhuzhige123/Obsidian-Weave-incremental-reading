...
vi.mock("../IRScheduleIndexService", () => ({
	getSharedIRScheduleIndexService: () => ({
		...
		getScheduleSources: async () => {
			...
			const { IRPdfBookmarkTaskService } = await import(
				"../IRPdfBookmarkTaskService"
			);
			const { IREpubBookmarkTaskService } = await import(
...
describe("IRPlanGeneratorService", () => {
	...
	function createItem(input: {
		...
		return {
			...
			topicKey:
				input.topicKey ??
				`source:${input.sourceFile ?? `/test/${input.id}.md`}`,
	...
	test("冻结窗口内的原定项目优先保留在原计划日", () => {
		...
		expect(
			plan.itemsByDate.get(formatDateKey(today))?.map((item) => item.id),
		).toEqual(["today-top"]);
		expect(
			plan.itemsByDate.get(formatDateKey(tomorrow))?.map((item) => item.id),
		).toEqual(["tomorrow-slot"]);
		expect(
			plan.itemsByDate
				.get(formatDateKey(dayAfterTomorrow))
				?.map((item) => item.id),
		).toEqual(["carryover"]);
	});
	...
	test("锁定到今天的项目即使超过当天预算也应保留在今天", () => {
		...
		expect(plan.itemsByDate.get(todayKey)?.map((item) => item.id)).toEqual([
			"auto-sub-1",
			"auto-sub-2",
		]);
		expect(
			plan.itemsByDate.get(formatDateKey(tomorrow))?.map((item) => item.id),
		).toEqual(["tomorrow-normal"]);
	});
	...
	test("标签组轻量偏置会在分数接近时影响排序，但不会改变调度单元", () => {
		...
		expect(
			plan.itemsByDate.get(formatDateKey(today))?.map((item) => item.id),
		).toEqual(["dense-group", "loose-group"]);
	});

	test("被预算顺延的材料会获得恢复加成，而原计划日材料不会收到这类补偿", () => {
		const today = createDate(0);
		const tomorrow = createDate(1);
		const carryover = createItem({
			id: "carryover",
			dayOffset: 0,
			priority: 5,
			estimatedMinutes: 10,
		});
		const tomorrowFresh = createItem({
			id: "tomorrow-fresh",
			dayOffset: 1,
			priority: 5,
			estimatedMinutes: 10,
	...
	test("当天有空位时未来阅读点不会自动前补到今天", () => {
		...
		expect(
			plan.itemsByDate
				.get(formatDateKey(twoDaysAfterTomorrow))
				?.map((item) => item.id),
		).toEqual(["gap-fill-2"]);
	});
	...
	test("remove_block 重算不会把明天的计划补到今天空位", async () => {
		...
		vi.spyOn(
			IRStorageService.prototype,
			"getAllChunkDataWithSync",
		).mockResolvedValue({
			"chunk-tomorrow": {
		...
		expect(
			normalPlan.itemsByDate
				.get(formatDateKey(tomorrow))
				?.map((item) => item.id),
		).toEqual(["chunk-tomorrow"]);
		expect(removedPlan.itemsByDate.get(formatDateKey(today))).toBeUndefined();
		expect(
			removedPlan.itemsByDate
				.get(formatDateKey(tomorrow))
				?.map((item) => item.id),
		).toEqual(["chunk-tomorrow"]);
	});
	...
	test("archive_block 重算也不会把未来计划补到今天", async () => {
		...
		const archivedPlan = await kernel.recomputeScheduleForDeck(
			"archive_block",
			{
				deckIds: ["deck-1"],
				horizonDays: 4,
	...
	test("启用负载顺延时按优先级保留 stretch 内项目并 defer 尾部", () => {
		...
		expect(plan.itemsByDate.get(tomorrowKey)?.map((item) => item.id)).toEqual([
			"low",
		]);
		expect(plan.loadDeferrals).toHaveLength(1);
		expect(plan.loadDeferrals[0]?.itemId).toBe("low");
		expect(
			plan.days.find((day) => day.dateKey === todayKey)?.overloadLevel,
		).toBe("overloaded");
	});
	...
	test("超出规划窗口仍未分配的项会写入 loadDeferrals", () => {
		...
		expect(
			overflowDeferrals.every((record) => record.action === "load_defer"),
		).toBe(true);
	});
});
