...
function mockYamlManager(overrides: Record<string, unknown> = {}): any {
	return {
		getReadingId: vi.fn(async () => null),
...
describe("ReadingMaterialManager", () => {
	it("md 文件创建阅读材料时应直接使用源文档，不复制到导入目录", async () => {
		...
		const yamlManager = mockYamlManager({ initializeReadingFields });
	...
	it("拆分 Markdown 导入时默认沿用上次手动选择的目录，而不是旧 importFolder", async () => {
		...
		const yamlManager = mockYamlManager();
	...
	it("拆分 Markdown 导入时应优先使用显式传入的 readableMarkdownFolder", async () => {
		...
		expect(imported[0].filePath).toContain(
			"Projects/Inbox/source/01_户曼卡片_纳博科夫卡片_思考流程对比.md",
		);
	...
	it("整文件副本导入时应在所选目录创建副本，并把 YAML 写入副本文件", async () => {
		...
		const material = await manager.createCopiedMarkdownMaterial(
			sourceFile as any,
			{
				source: "manual",
				category: ReadingCategory.Later,
	...
	it("setReadingDeck 时应同步把专题写回 markdown 文件的 YAML", async () => {
		...
		const app = {
			vault: {
				getAbstractFileByPath: vi.fn((path: string) =>
					path === markdownFile.path ? markdownFile : null,
				),
			},
		};

		const storage = {
			getMaterialById: vi.fn((uuid: string) =>
				uuid === material.uuid ? material : null,
			),
			saveMaterial: vi.fn(async () => {}),
		};
		...
		expect(yamlManager.updateReadingFields).toHaveBeenCalledWith(markdownFile, {
			"weave-reading-topic-id": "deck-1",
		});
	});
	...
	it("从全局侧边栏删除 Markdown 阅读材料时应清理外部文档调度残留", async () => {
		...
		const app = {
			vault: {
				getAbstractFileByPath: vi.fn((path: string) =>
					path === material.filePath ? file : null,
				),
			},
		};
		...
		const initializeSpy = vi
			.spyOn(IRStorageService.prototype, "initialize")
			.mockResolvedValue(undefined);
		const removeMaterialScheduleDataSpy = vi
			.spyOn(IRStorageService.prototype, "removeMaterialScheduleData")
			.mockResolvedValue(undefined);
		const cleanupRemovedMaterialDocumentSpy = vi
			.spyOn(IRStorageService.prototype, "cleanupRemovedMaterialDocument")
			.mockResolvedValue(undefined);
		...
		try {
			...
			expect(removeMaterialScheduleDataSpy).toHaveBeenCalledWith(
				material.filePath,
			);
			expect(cleanupRemovedMaterialDocumentSpy).toHaveBeenCalledWith(
				material.filePath,
				{
	...
	it("设置多关联笔记时应同步主笔记与兼容旧字段", async () => {
		...
		expect((material as any).associatedNotePaths).toEqual([
			"Folder/Topic.md",
			"Folder/Appendix.md",
		]);
		expect(storage.saveMaterial).toHaveBeenCalledWith(
	...
	it("Markdown 阅读材料不再接受关联笔记写入", async () => {
		const material = {
			uuid: "mat-md",
			filePath: "notes/source.md",
			title: "source",
		...
		const success = await manager.setAssociatedNotePaths("mat-md", [
			"Folder/Topic.md",
		]);
		expect(success).toBe(false);
		expect(storage.saveMaterial).not.toHaveBeenCalled();
	});
});
