	...
	describe("预算控制", () => {
		test("不超过时间预算", () => {
			const blocks = Array.from(
				{ length: 50 },
				(_, i) => createTestBlock(`block${i}`, "A", 5, 5), // 每块 5 分钟
			);
			const groupMapping: Record<string, string> = {};
			blocks.forEach((b) => {
				groupMapping[b.id] = "A";
	...
	describe("软交错倾向", () => {
		...
		test("超过软阈值后会倾向切换到其他仍有候选的主题", () => {
			...
			const groupMapping = Object.fromEntries(
				blocks.map((block) => [block.id, block.meta?.tagGroup || "default"]),
			);
			...
			const sequence = result.queue.map(
				(block) => block.meta?.tagGroup || "default",
			);
