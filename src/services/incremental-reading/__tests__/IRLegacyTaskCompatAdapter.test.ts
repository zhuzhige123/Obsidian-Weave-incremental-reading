...
describe("IRLegacyTaskCompatAdapter", () => {
	it("buildLegacyBlockFromPointSnapshot 不再为 Markdown 阅读点暴露关联笔记", () => {
		const block = buildLegacyBlockFromPointSnapshot(createSnapshot()) as any;

		expect(block).toBeTruthy();
		expect(block?.primaryAssociatedNotePath).toBeUndefined();
		expect(block?.associatedNotePath).toBeUndefined();
		expect(block?.associatedNotePaths).toBeUndefined();
		expect(block?.meta?.associatedNotePaths).toBeUndefined();
	});
