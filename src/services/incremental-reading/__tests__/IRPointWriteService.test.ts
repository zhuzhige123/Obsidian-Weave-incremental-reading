...
describe("IRPointWriteService", () => {
	...
	it("拒绝为 Markdown chunk 写入关联笔记", async () => {
		...
		expect(storageSpies.saveChunkData).not.toHaveBeenCalled();
	});
	...
	it("删除 chunk 点在运行时缺失时会回退到 legacy points 删除", async () => {
		storageSpies.getChunkData.mockResolvedValue(null);
		storageSpies.deleteLegacyPointById.mockResolvedValue(true);
		const service = new IRPointWriteService({} as any);

		const result = await service.deletePoint({
			id: "chunk-q4t5dnb4p3yg",
			kind: "chunk",
		});
	...
	it("删除已不存在的 chunk 点会幂等成功并刷新调度缓存", async () => {
		storageSpies.getChunkData.mockResolvedValue(null);
		storageSpies.deleteLegacyPointById.mockResolvedValue(false);
		const service = new IRPointWriteService({} as any);
		...
		expect(storageSpies.deleteChunkData).not.toHaveBeenCalled();
		expect(storageSpies.invalidateScheduleRuntimeCaches).toHaveBeenCalled();
		expect(result).toBe(true);
	});
});
