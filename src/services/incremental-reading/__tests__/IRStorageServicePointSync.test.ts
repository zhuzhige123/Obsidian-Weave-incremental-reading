const {
	pointStorageState,
	pointStorageSpies,
	sharedMockPointStorage,
	MockIRPointStorageService,
} = vi.hoisted(() => {
	const pointStorageState = {
		pointDecks: {} as Record<string, any>,
		pointSnapshots: [] as any[],
		snapshotCacheVersion: 0,
	};

	const bumpSnapshotCacheVersion = () => {
	...
	class MockIRPointStorageService {
		...
		getSnapshotListCacheVersion = () => pointStorageState.snapshotCacheVersion;
		invalidatePointSnapshotListCache = vi.fn(() => {
			bumpSnapshotCacheVersion();
		});
	}

	const sharedMockPointStorage = new MockIRPointStorageService();
...
vi.mock("../IRPointStorageService", () => ({
	IRPointStorageService: MockIRPointStorageService,
	getSharedIRPointStorageService: () => sharedMockPointStorage,
}));
...
describe("IRStorageService point sync", () => {
	...
	it("reads legacy blocks from migrated point snapshots and writes back into point storage", async () => {
		...
		expect(blocks["legacy-block-1"]?.primaryAssociatedNotePath).toBeUndefined();
