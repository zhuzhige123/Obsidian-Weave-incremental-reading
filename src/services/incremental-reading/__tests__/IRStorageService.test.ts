...
describe("IRStorageService.deleteChunkData", () => {
	it("删除最后一个同源阅读块时应清理源 Markdown 的增量阅读 frontmatter 并追加已删除标签", async () => {
		...
		const saveDeck = vi
			.spyOn(service as any, "saveDeck")
			.mockResolvedValue(undefined);
		const deleteFileSyncState = vi
			.spyOn(service as any, "deleteFileSyncState")
			.mockResolvedValue(undefined);
		vi.spyOn(
			service as any,
			"deleteChunkPointFromNewStorage",
		).mockResolvedValue(undefined);
	...
	it("删除外部文档型阅读点时应直接清理当前 md 文件中的 IR frontmatter", async () => {
		...
		const app = {
			...
			vault: {
				getAbstractFileByPath: vi.fn((filePath: string) =>
					filePath === "notes/external-ir.md" ? externalFile : null,
				),
			},
			fileManager: {
				processFrontMatter: vi.fn(
...
describe("IRStorageService.deleteDeck", () => {
	it("删除专题时应保留 Markdown 源文档，仅清理增量阅读 frontmatter 并追加已删除标签", async () => {
		...
		const app = {
			...
			vault: {
				getAbstractFileByPath: vi.fn((filePath: string) =>
					filePath === "notes/topic-source.md" ? sourceFile : null,
				),
				adapter: {
					remove,
		...
		const deletePointDeck = vi.fn(async () => ({
			removed: true,
			topicName: "测试专题",
			pointIds: [],
			sourceFiles: [],
		...
		vi.spyOn(
			IRPointWriteService.prototype,
			"deletePointsByDeckIdentifiers",
		).mockResolvedValue(0);
		vi.spyOn(
			IRPointWriteService.prototype,
			"deletePdfPointsByPaths",
		).mockResolvedValue(0);
		vi.spyOn(
			IRPointWriteService.prototype,
			"deleteEpubPointsByPaths",
		).mockResolvedValue(0);
	...
	it("删除专题时即使只剩外部文档字段也应清理并追加已删除标签", async () => {
		...
		const app = {
			...
			vault: {
				getAbstractFileByPath: vi.fn((filePath: string) =>
					filePath === "notes/topic-external.md" ? sourceFile : null,
				),
				adapter: {
					remove: vi.fn(async () => {}),
					exists: vi.fn(async () => false),
