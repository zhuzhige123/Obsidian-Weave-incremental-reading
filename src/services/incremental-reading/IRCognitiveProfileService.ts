...
export class IRCognitiveProfileService {
	computeProfile(input: IRCognitiveProfileInput): IRCognitiveProfile {
		...
		const effectivePriority = clampScore(
			input.effectivePriority ?? input.manualPriority ?? 5,
		);
		const memoryPrioritySignalScore = roundScore(
			clampScore(input.memoryPrioritySignal ?? 0),
		);
		const hasMemoryPrioritySignal =
			typeof input.memoryPrioritySignal === "number";
		...
		const urgencyScore = roundScore(
			clampScore(urgencyBase + overdueDays * 1.5),
		);
		const intervalFactor =
			input.intervalDays && input.intervalDays > 1
				? Math.min(2, input.intervalDays / 7)
				: 0;
		...
		const stalenessScore = roundScore(
			clampScore(
				(overdueDays > 0 ? Math.min(4, overdueDays) : 0) +
					Math.min(3, intervalFactor),
			),
		);
		...
		const fatiguePenalty = roundScore(
			clampScore(
				input.estimatedMinutes >= 8 ? 2 : input.estimatedMinutes >= 5 ? 1 : 0,
			),
		);
	...
	computeUtilityBreakdown(
		...
		const fatiguePenalty = clampScore(
			context?.fatiguePenalty ?? profile.fatiguePenalty,
		);
		const volatilityPenalty = clampScore(
			context?.volatilityPenalty ?? profile.volatilityPenalty,
		);
		const contributions = {
