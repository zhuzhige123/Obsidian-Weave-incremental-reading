...
export function resolveScheduleItemWebUrl(app: App, material: ScheduleItem): string | null {
	...
	const frontmatterUrl = String(cache?.frontmatter?.[IR_WEB_URL_FRONTMATTER_KEY] || "").trim();
...
export function buildWebReadingPointMarkdown(
	...
	const safeTitle = String(title || "").trim() || deriveWebPageTitleFromUrl(url) || "网页阅读点";
