...
export class IRScheduleKernel {
	...
	> {
		...
		const materialPaths = resolveAssociatedNotePaths({
	...
	private async loadScheduleCandidatesFromStorage(): Promise<{
	...
	private async loadScheduleCandidates(): Promise<{
		chunks: IRChunkFileData[];
		pdfTasks: IRPdfBookmarkTask[];
		epubTasks: IREpubBookmarkTask[];
	}> {
