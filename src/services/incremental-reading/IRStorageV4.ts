...
export class IRStorageV4 {
	...
	async initialize(): Promise<void> {
		...
		try {
			...
			// 加载数据
			await Promise.all([this.loadBlocks(), this.loadDecks(), this.loadHistory()]);
	...
	async getBlocksByFile(filePath: string): Promise<IRBlockV4[]> {
		await this.initialize();
		return Object.values(this.blocksCache).filter((b) => b.sourcePath === filePath);
	}
	...
	async getDueBlocks(deckId: string): Promise<IRBlockV4[]> {
		...
		return blocks.filter((_block) => {
			// 新块总是到期
			if (_block.status === "new") return true;

			// scheduled 状态：检查时间
			if (_block.status === "scheduled") {
				return _block.nextRepDate === 0 || _block.nextRepDate <= now;
			}

			// queued 状态：也检查时间
			if (_block.status === "queued") {
				return _block.nextRepDate === 0 || _block.nextRepDate <= now;
			}
	 ...
	 */
	async getDeckStats(deckId: string, learnAheadDays = 3): Promise<IRDeckStats> {
		const blocks = await this.getBlocksByDeck(deckId);
		const now = Date.now();
