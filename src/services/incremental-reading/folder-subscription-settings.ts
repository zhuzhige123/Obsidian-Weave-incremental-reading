...
export function createIncrementalReadingFolderSubscriptionRuleId(): string {
	return `ir-folder-sub-${Date.now().toString(36)}-${Math.random()
		.toString(36)
		.slice(2, 8)}`;
}

export function normalizeIncrementalReadingFolderSubscriptionPath(
	folderPath: string,
): string {
...
export function normalizeIncrementalReadingFolderSubscriptionRule(
	...
	return {
		...
		folderPath: normalizeIncrementalReadingFolderSubscriptionPath(
...
export function normalizeIncrementalReadingFolderSubscriptionSettings(
	...
	const normalizedRules = Array.isArray(settings?.rules)
		? settings.rules.map((rule, index) =>
				normalizeIncrementalReadingFolderSubscriptionRule(rule, index),
		  )
		: [];
	...
	return {
		rules: migratedRules.filter(
			(rule) => rule.enabled || rule.folderPath || rule.deckId,
		),
		initialScheduleMode,
		importConfirmThreshold:
			Number.isFinite(threshold) && threshold >= 0
				? Math.min(200, Math.round(threshold))
				: 20,
	};
}
...
export function getActiveIncrementalReadingFolderSubscriptionRules(
	...
	return (
		normalizeIncrementalReadingFolderSubscriptionSettings(settings).rules || []
	).filter(
		(rule) =>
			rule.enabled === true && Boolean(rule.folderPath) && Boolean(rule.deckId),
	);
}

export function isFileWithinIncrementalReadingFolderSubscription(
	filePath: string,
	folderPath: string,
): boolean {
...
export function resolveIncrementalReadingFolderSubscriptionRuleForFile(
	...
	const rules = Array.isArray(settingsOrRules)
		? settingsOrRules.map((rule, index) =>
				normalizeIncrementalReadingFolderSubscriptionRule(rule, index),
		  )
	...
	const matchedRules = rules
		...
		.filter(
			({ rule }) =>
				rule.enabled === true &&
				isFileWithinIncrementalReadingFolderSubscription(
					filePath,
					rule.folderPath || "",
				),
		)
