...
function snapshotSupportsPointLinkedNotes(snapshot: IRPointSnapshot): boolean {
	...
	const originType = snapshot.point.audit?.origin?.type;
	if (supportsPointLinkedNotes(originType)) {
		return true;
	}

	const locatorType = snapshot.point.trace?.locatorType;
	if (locatorType === "pdf-selection" || locatorType === "epub-chapter") {
		return true;
	}
...
function resolvePointAssociatedNotePaths(snapshot: IRPointSnapshot): string[] {
	if (!snapshotSupportsPointLinkedNotes(snapshot)) {
		return [];
	}
