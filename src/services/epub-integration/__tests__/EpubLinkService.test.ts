...
const encodeCompactField = (value: string): string =>
	Buffer.from(value, "utf8").toString("base64url");

const buildCompactReadiumLocator = (
	href: string,
	progression: string,
	highlight?: string,
): string => {
	const fields = [
		"loc",
		encodeCompactField(href),
		progression,
		"",
		"",
		highlight ? encodeCompactField(highlight) : "",
	];
...
describe("EpubLinkService legacy link compatibility", () => {
	it("extracts the first EPUB wikilink for both current and legacy hash formats", () => {
		const current =
			"前文 [[Books/demo.epub#weave-cfi=readium%3Aabc|Demo]] 后文";
		const legacyDash =
			"前文 [[Books/demo.epub#tuanki-cfi-epubcfi(/6/2[chapter-1]!/4/4)|Demo]] 后文";
		const legacyEquals =
			"前文 [[Books/demo.epub#tuanki-cfi=epubcfi(/6/2[chapter-1]!/4/4)|Demo]] 后文";

		expect(EpubLinkService.extractFirstEpubLinkMarkup(current)).toBe(
			"[[Books/demo.epub#weave-cfi=readium%3Aabc|Demo]]",
		);
		expect(EpubLinkService.extractFirstEpubLinkMarkup(legacyDash)).toBe(
			"[[Books/demo.epub#tuanki-cfi-epubcfi(/6/2[chapter-1]!/4/4)|Demo]]",
		);
		expect(EpubLinkService.extractFirstEpubLinkMarkup(legacyEquals)).toBe(
			"[[Books/demo.epub#tuanki-cfi=epubcfi(/6/2[chapter-1]!/4/4)|Demo]]",
		);
	});
	...
	it("extracts legacy protocol EPUB links and resolves their file paths", () => {
		...
		expect(EpubLinkService.extractFirstEpubLinkMarkup(protocolLink)).toBe(
			protocolLink,
		);
		expect(
			EpubLinkService.extractFilePathFromEpubLinkMarkup(protocolLink),
		).toBe("Books/demo.epub");
	});

	it("parses both legacy tuanki subpaths and readium subpaths", () => {
		expect(
			EpubLinkService.parseEpubLink(
				"#tuanki-cfi-epubcfi(/6/2[chapter-1]!/4/4)",
			),
		).toEqual({
			filePath: "",
		...
		expect(
			EpubLinkService.parseEpubLink(
				"#weave-cfi=readium%3Aabc&chapter=3&text=Hello%20world",
			),
		).toEqual({
			filePath: "",
			cfi: "readium:abc",
	...
	it("parses complete EPUB link markup for both current wikilinks and legacy protocol links", () => {
		expect(
			EpubLinkService.parseLinkMarkup(
	...
	it("preserves source identity inside epub source links", () => {
		...
		expect(built).toMatch(
			/^\[\[Books\/demo\.epub#weave-cfi=.*&sid=epubsrc-fixed\|demo\]\]$/,
		);
		expect(EpubLinkService.parseLinkMarkup(built)).toEqual({
			filePath: "Books/demo.epub",
			cfi: "epubcfi(/6/2)",
	...
	it("keeps compact readium locators short without duplicating text payloads", () => {
		const service = new EpubLinkService({} as any);
		const compactLocator = buildCompactReadiumLocator(
		...
		expect(
			EpubLinkService.extractEmbeddedTextFromReadiumLocator(compactLocator),
		).toBe("Hello world");
		expect(
			EpubLinkService.parseEpubLink(`#weave-cfi=${compactLocator}&chapter=3`),
		).toEqual({
			filePath: "",
			cfi: compactLocator,
			text: "Hello world",
	...
	it("builds and parses combined highlight color and style metadata", () => {
		expect(EpubLinkService.buildHighlightCalloutMeta("blue", "underline")).toBe(
			"blue+underline",
		);
		expect(EpubLinkService.buildHighlightCalloutMeta("pink", "wavy")).toBe(
			"red+wavy",
		);
	...
	it("detects and migrates legacy epub links inside content", () => {
		...
		expect(
			EpubLinkService.isLegacyEpubLinkMarkup(
				"[[Books/demo.epub#weave-cfi=readium:xyz|demo]]",
			),
		).toBe(false);
		...
		expect(
			EpubLinkService.parseLinkMarkup(lines[0].replace(/^前文 /, "")),
		).toEqual({
			filePath: "Books/demo.epub",
			cfi: "readium:abc",
			text: "",
	...
	it("enriches existing epub links with source ids without changing the locator", async () => {
		...
		expect(result.content).toMatch(
			/\[\[Books\/demo\.epub#weave-cfi=readium:abc&sid=epubsrc-/,
		);
	});
	...
	it("uses pendingCfi when navigating from stored resume cfi", async () => {
		...
		vi.doMock("../ir-epub-storage-access", () => ({
			getIrEpubStorageService: () => ({
				resolveSourceFilePath: async (_sourceId?: string, filePath?: string) =>
					filePath || "",
			}),
		}));

		const { EpubLinkService: ScheduleEpubLinkService } = await import(
			"../EpubLinkService"
		);
