...
export class IrEpubSourceRegistry implements IrEpubStorageLike {
			...
			pluginCandidates
				.map(
					(plugin) =>
						(
							plugin as
								| { settings?: { weaveParentFolder?: string } }
								| undefined
						)?.settings?.weaveParentFolder,
				)
				.find((folder): folder is string =>
					Boolean(folder && String(folder).trim()),
				),
		);
		this.basePath = getV2Paths(parentFolder).ir.epub;
		this.unifiedDataPath = normalizePath(
			getPluginPathsById(app, runtime.pluginId).state.incrementalReading
				.epubReaderData,
		);
	}
	...
	async ensureSourceIdentity(
		...
		const sourceFingerprint =
			await this.computeSourceFingerprint(normalizedPath);
	...
	async resolveSourceFilePath(
		...
		if (sourceId) {
			...
			if (
				...
				(await this.hasExistingBookFile(registryEntry.filePath))
			) {
				return registryEntry.filePath;
			}
			if (
				normalizedFallback &&
				(await this.hasExistingBookFile(normalizedFallback))
			) {
				await this.ensureSourceIdentity(normalizedFallback, {
	...
	private async saveSourceRegistry(
		entries: EpubSourceRegistryEntry[],
	): Promise<void> {
	...
	private async readLegacySourceRegistry(): Promise<
		EpubSourceRegistryEntry[] | null
	> {
		...
		try {
			...
			logger.warn(
				"[IrEpubSourceRegistry] Failed to read epub-source-registry.json:",
				error,
			);
			return null;
		}
	...
	private async readUnifiedLocalReaderData(): Promise<EpubReaderLocalDataFile> {
		...
		try {
			...
			this.localDataCache = {
				...
				sourceRegistry: Array.isArray(parsed.sourceRegistry)
					? parsed.sourceRegistry
					: undefined,
			};
		} catch (error) {
			logger.warn(
				"[IrEpubSourceRegistry] Failed to parse epub-reader-data.json:",
				error,
			);
			this.localDataCache = { version: 1, updatedAt: 0 };
		}
	...
	private normalizeSourceRegistryEntries(
		value: unknown,
	): EpubSourceRegistryEntry[] {
	...
	private generateSourceId(): string {
		return `epubsrc-${Date.now().toString(36)}${Math.random()
			.toString(36)
			.slice(2, 8)}`;
	}

	private async computeSourceFingerprint(
		filePath: string,
	...
	private async getExistingFileStat(
		filePath: string,
