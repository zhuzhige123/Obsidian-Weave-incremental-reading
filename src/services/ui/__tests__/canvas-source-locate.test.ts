...
describe("canvas-source-locate", () => {
	it("normalizes canvas node ids from mixed source formats", () => {
		...
		expect(normalizeCanvasNodeId("canvas-file-node:node-4")).toBe("node-4");
	...
	it("resolves canvas node ids from menu node wrappers", () => {
		expect(
			resolveCanvasMenuNodeId({
				id: "wrapper-node",
				getData: () => ({ id: "data-node", type: "text", text: "正文" }),
			}),
		).toBe("wrapper-node");
		expect(
			resolveCanvasMenuNodeId({
				getData: () => ({
					id: "canvas-node:node-5",
					type: "text",
					text: "正文",
				}),
			}),
	...
	it("extracts canvas node id and rect directly from source links", () => {
		const sourceLink =
			"[[boards/topic.canvas#^node-9?x=12&y=34&w=56&h=78|节点]]";
