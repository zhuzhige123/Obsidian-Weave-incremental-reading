...
describe("SourceLocateOverlayService", () => {
	...
	it("shows the locate overlay at the matched text rect when markdown text is available", () => {
		...
		const showSpy = vi
			.spyOn(service, "showAtRect")
			.mockImplementation(() => true);

		const shown = service.showForMarkdownTarget(
			container,
			["每个台阶能发出不同的音符"],
			{
				label: "定位到溯源位置",
	...
	it("locates EPUB callouts in source editor mode and anchors to the matched quote line", () => {
		...
		vi.spyOn(quoteLine, "getBoundingClientRect").mockReturnValue(
			new DOMRect(48, 180, 920, 30),
		);
		vi.spyOn(Range.prototype, "getBoundingClientRect").mockImplementation(
			function (this: Range) {
	...
	it("uses viewport positioning even when the input rect only provides x and y", () => {
		...
		const overlay = document.body.querySelector(
			".weave-source-locate-overlay",
		) as HTMLElement | null;
		expect(overlay).not.toBeNull();
		expect(overlay?.classList.contains("weave-source-locate-overlay")).toBe(
			true,
		);
	...
	it("does not render the locate overlay when rect coordinates are invalid", () => {
		...
		expect(
			document.body.querySelector(".weave-source-locate-overlay"),
		).toBeNull();
	});
});
