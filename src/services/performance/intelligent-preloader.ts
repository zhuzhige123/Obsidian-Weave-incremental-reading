...
export class UsagePatternAnalyzer {
	 ...
	 * 预测下一个可能访问的资源
	 */
	predictNextResources(currentContext: UsageContext, limit = 10): PredictionResult[] {
	...
	getResourceFrequency(resourceId: string, timeWindow?: number): number {
		...
		const relevantRecords = this.usageHistory.filter(
			(record) => record.resourceId === resourceId && record.timestamp >= windowStart
		);
			...
			if (sequenceCount > 0) {
				...
				if (sequenceProbability > 0.05) {
					...
					predictions.push({
						...
						priority:
							sequenceProbability > 0.4 ? "high" : sequenceProbability > 0.2 ? "medium" : "low",
