...
export class IntelligentBatchProcessor {
	...
	async processBatch<T, R>(
		...
		logger.debug(
			`📊 初始配置: 批量大小=${this.currentBatchSize}, 并发数=${this.currentConcurrency}`
		);
		...
		const batchPromises = batches.map(async (batch, index) => {
			...
			try {
				...
				const batchResults = await this.processSingleBatch(batch, processor, index, options);
	...
	private async adaptiveOptimization(): Promise<void> {
		...
		const avgThroughput =
			recentHistory.reduce((sum, h) => sum + h.throughput, 0) / recentHistory.length;
		const avgErrorRate =
			recentHistory.reduce((sum, h) => sum + h.errorRate, 0) / recentHistory.length;
		...
		if (isErrorRateHigh) {
			...
			adaptationMade = true;
			logger.debug(
				`🔧 高错误率调整: 并发 ${oldConcurrency}→${this.currentConcurrency}, 批量 ${oldBatchSize}→${this.currentBatchSize}`
			);
		...
		if (adaptationMade) {
			this.adaptations.push({
				timestamp: Date.now(),
				reason: isErrorRateHigh
					? "high-error-rate"
					: isPerformanceDecreasing
