		...
		fc.assert(
			fc.property(
				...
				(priority, days1, days2, decayRate, category) => {
					...
					// 更多天数应该导致更低或相等的优先级
					expect(priorityAfterMoreDays).toBeLessThanOrEqual(
						priorityAfterFewerDays,
					);
		...
		fc.assert(
			fc.property(
				...
				(priority, days, lowerRate, higherRate, category) => {
					...
					// 更高的衰减率应该导致更低或相等的优先级
					expect(priorityWithHigherRate).toBeLessThanOrEqual(
						priorityWithLowerRate,
					);
...
describe("优先级有效性", () => {
	...
	it("极端衰减应该将优先级降至0", () => {
		fc.assert(
			fc.property(priorityArb, nonFavoriteCategoryArb, (priority, category) => {
				// 使用极高的衰减率和天数
				const newPriority = calculatePriorityDecay(
					priority,
					1000, // 1000天
