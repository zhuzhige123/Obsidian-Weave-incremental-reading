...
describe("Property 3: FSRS6 Scheduling Consistency", () => {
	...
	it("调度后的due日期应该在未来", () => {
		fc.assert(
			fc.property(fsrsCardArb, ratingArb, (card, rating) => {
				const now = new Date();
	...
	it("Easy评分的间隔应该大于Good评分", () => {
		fc.assert(
			fc.property(fsrsCardArb, (card) => {
				...
				expect(easyScheduled.scheduledDays).toBeGreaterThanOrEqual(
					goodScheduled.scheduledDays,
				);
	...
	it("Good评分的间隔应该大于Hard评分", () => {
		fc.assert(
			fc.property(fsrsCardArb, (card) => {
				...
				expect(goodScheduled.scheduledDays).toBeGreaterThanOrEqual(
					hardScheduled.scheduledDays,
				);
...
describe("阅读材料与卡片调度一致性", () => {
	it("阅读材料和卡片使用相同的FSRS算法", () => {
		...
		// 模拟阅读材料完成（使用相同的调度器）
		const materialAfterReading = simulateFSRSSchedule(
			initialCard,
			RatingEnum.Good,
		);
		...
		expect(cardAfterReview.scheduledDays).toBe(
			materialAfterReading.scheduledDays,
		);
