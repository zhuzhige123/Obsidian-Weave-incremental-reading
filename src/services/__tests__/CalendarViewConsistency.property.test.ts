 ...
 */
function countMaterialsForDate(
 ...
 * 生成有效的FSRS卡片状态
 */
const fsrsCardArb = (dueDate: Date): fc.Arbitrary<FSRSCard> =>
	fc
		.record({
			stability: fc.float({
				min: Math.fround(0.1),
 ...
 * 生成有效的阅读进度
 */
const progressArb: fc.Arbitrary<ReadingProgress> = fc
	.record({
 ...
 * 生成一组阅读材料（指定due日期分布）
 */
const materialsWithDistributionArb = fc
	.array(
		fc.tuple(
	...
	.chain((distribution) => {
		const materialArrays = distribution.map(([dayOffset, count]) => {
			...
			date.setHours(0, 0, 0, 0);
			return fc.array(materialWithDueDateArb(date), {
				minLength: count,
				maxLength: count,
			});
		});
...
describe("Property 9: Calendar View Consistency", () => {
	...
	it("日历视图中的材料数量应该与FSRS.due日期匹配", () => {
		fc.assert(
			fc.property(materialsArb, futureDateArb, (materials, targetDate) => {
				// 计算日历视图显示的数量
	...
	it("日历网格中所有日期的计数总和应该等于有due日期的材料总数", () => {
		fc.assert(
			fc.property(materialsWithDistributionArb, (materials) => {
				const now = new Date();
		...
		fc.assert(
			fc.property(
				...
				(materials, dayOffset) => {
					...
					const selectedMaterials = getMaterialsForDate(
						materials,
						selectedDate,
					);
