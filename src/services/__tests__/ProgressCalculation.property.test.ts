...
const anchorPositionsArb = (maxPosition: number) =>
	fc
		.array(fc.integer({ min: 0, max: maxPosition }), {
			minLength: 1,
			maxLength: 10,
		})
		...
		fc.assert(
			fc.property(
				...
				(totalWords, ratio) => {
					...
					expect(Math.abs(percentage - expectedPercentage)).toBeLessThanOrEqual(
						1,
					);
	...
	it("锚点历史记录应该保持时间顺序", () => {
		fc.assert(
			fc.property(
				...
				(anchorData) => {
					// 按时间戳排序
					const sortedByTime = [...anchorData].sort(
...
describe("进度更新属性", () => {
	...
	it("进度变化应该与字数变化成正比", () => {
		fc.assert(
			fc.property(fc.integer({ min: 1000, max: 10000 }), (totalWords) => {
				...
				increments.forEach((inc, i) => {
					...
					expect(
						Math.abs(progresses[i] - expectedProgress),
