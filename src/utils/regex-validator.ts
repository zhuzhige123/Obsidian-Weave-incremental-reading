 ...
 * 检测危险的正则表达式模式
 */
function detectDangerousPatterns(pattern: string): {
	warnings: string[];
	criticalIssues: string[];
	riskLevel: "low" | "medium" | "high" | "critical";
} {
 ...
 * 高级正则表达式性能测试
 */
async function testRegexPerformanceAdvanced(
	pattern: string,
	timeoutMs: number
): Promise<{
	passed: boolean;
...
export async function validateRegex(
	...
	// 根据风险级别决定是否通过验证
	const shouldReject =
		dangerousPatterns.riskLevel === "critical" ||
		(dangerousPatterns.riskLevel === "high" &&
			performanceTest.maxExecutionTime > opts.timeoutMs * 0.5);
	...
	return {
		...
		suggestions:
			dangerousPatterns.warnings.length > 0 ? getRegexSecurityAdvice(pattern) : undefined,
	};
}
 ...
 * 同步版本的正则表达式验证（向后兼容）
 */
export function validateRegexSync(
	pattern: string,
	options: RegexSecurityOptions = {}
	...
	// 简单的性能测试
	const testStrings = ["a".repeat(100), "ab".repeat(50), "abc".repeat(33), "1234567890".repeat(10)];
