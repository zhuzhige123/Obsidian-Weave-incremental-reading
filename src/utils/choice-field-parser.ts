	...
	private static validateOptionSequence(options: ChoiceOption[]): {
		...
		if (isLetterSequence) {
			...
			if (JSON.stringify(expectedSequence) !== JSON.stringify(numbers)) {
				warnings.push(
					`选项编号不连续，期望: ${expectedSequence.join(", ")}，实际: ${numbers.join(", ")}`
				);
			}
		} else {
