...
export class ErrorHandler {
	...
	handleError(
		...
		const { duration = 5000, level = "error", showInConsole = true } = options;
	 ...
	 * 格式化错误信息
	 */
	private formatErrorMessage(error: unknown, message: string, context?: ErrorContext): string {
	...
	private logToConsole(
		...
		if (level === "error") {
			logger.error("[Weave Plugin]", logData);
			return;
		}
		if (level === "warning") {
			logger.warn("[Weave Plugin]", logData);
			return;
		}
		logger.info("[Weave Plugin]", logData);
	}
}
 ...
 * 便捷的错误处理函数
 */
export function handleError(error: unknown, message: string, context?: ErrorContext): void {
