...
export function isValidLicenseType(value: unknown): value is LicenseType {
	return (
		typeof value === "string" && ["lifetime", "subscription"].includes(value)
	);
}
 ...
 * 检查是否为有效的激活错误代码
 */
export function isValidActivationErrorCode(
	value: unknown,
): value is ActivationErrorCode {
	return (
		typeof value === "string" &&
 ...
 * 检查是否为有效的设备指纹
 */
export function isValidDeviceFingerprint(
	value: unknown,
): value is DeviceFingerprint {
