  ...
  const plugin = {
    ...
    getEffectiveLicenseState() {
      return resolveEffectiveLicenseState({
        ...
        inheritedLicenses:
          plugin.settings.allowInheritedLicenses === false ? [] : inheritedLicenses,
      });
    },
  };
...
describe("plugin-license", () => {
  ...
  it("identifies inherited-only removal when no local license exists but shared access is active", () => {
    ...
    expect(getPluginActivationRemovalKind(plugin, { disableInheritedLicenses: true })).toBe(
      "inherited-only"
    );
  ...
  it("keeps the latest local license as the legacy primary mirror after upsert", () => {
    ...
    expect(
      plugin.settings.licenseState.localLicenses.map((license) => license.activationCode)
