...
describe("vaultStorage", () => {
	...
	it("persists managed keys to file while leaving non-plugin keys in Obsidian localStorage", async () => {
		...
		vaultStorage.setItem("weave-ir-calendar-filter", "topic-a");
		...
		expect(
			...
			"weave-ir-calendar-filter": "topic-a",
		});
		expect(window.localStorage.getItem("language")).toBe("en");
		expect(window.localStorage.getItem("weave-ir-calendar-filter")).toBeNull();

		vaultStorage.removeItem("weave-ir-calendar-filter");
		await vaultStorage.flush();
		...
		expect(vaultStorage.getItem("weave-ir-calendar-filter")).toBeNull();
		expect(window.localStorage.getItem("language")).toBe("en");
	});
});
