...
describe("ir-internal-data-path", () => {
	it("detects .irdeck paths case-insensitively", () => {
		expect(isIRDeckFilePath("weave/incremental-reading/points/pdf.irdeck")).toBe(true);
	...
	it("sanitizes internal paths to empty", () => {
		expect(sanitizeUserReadingSourcePath("weave/incremental-reading/points/pdf.irdeck")).toBe("");
	...
	it("strips any extension for display basename", () => {
		...
		expect(basenameWithoutExtension("Inbox/01_Chunk.md", "fallback")).toBe("01_Chunk");
	});

	it("flags schedule items backed by irdeck source files", () => {
		expect(shouldExcludeScheduleItemBySource({ sourceFile: "points/pdf.irdeck" })).toBe(true);
		expect(shouldExcludeScheduleItemBySource({ sourceFile: "Books/demo.pdf" })).toBe(false);
	});
});
