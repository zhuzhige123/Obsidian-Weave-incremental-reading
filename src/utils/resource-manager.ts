...
export class EditorResourceManager {
	...
	registerTimer(
		...
		logger.debug(
			`[ResourceManager] 注册定时器 [${this.editorId}]: ${resourceId} (${description || "unknown"})`
		);
		return resourceId;
	}
	...
	registerEventListener(
		...
		logger.debug(
			`[ResourceManager] 注册事件监听器 [${this.editorId}]: ${resourceId} (${type} - ${
				description || "unknown"
			})`
		);
		return resourceId;
	...
	registerPromise(
		...
		logger.debug(
			`[ResourceManager] 注册Promise [${this.editorId}]: ${resourceId} (${
				description || "unknown"
			})`
		);
		return resourceId;
	...
	registerComponent(component: any, cleanup: ResourceCleanup, description?: string): string {
		...
		logger.debug(
			`[ResourceManager] 注册组件 [${this.editorId}]: ${resourceId} (${description || "unknown"})`
		);
		return resourceId;
	}
	...
	registerCustomCleanup(cleanup: ResourceCleanup, description?: string): string {
		...
		logger.debug(
			`[ResourceManager] 注册清理函数 [${this.editorId}]: ${resourceId} (${
				description || "unknown"
			})`
		);
		return resourceId;
	...
	destroy(): void {
		...
		for (const [resourceId, listener] of this.eventListeners) {
			try {
				listener.target.removeEventListener(listener.type, listener.listener, listener.options);
				logger.debug(
					`[ResourceManager] 清理事件监听器 [${this.editorId}]: ${resourceId} (${listener.type} - ${
						listener.description || "unknown"
					})`
				);
			} catch (error) {
				logger.error(
					`[ResourceManager] 事件监听器清理失败 [${this.editorId}]: ${resourceId}`,
					error
				);
			}
		}
		...
		for (const [resourceId, promiseResource] of this.promises) {
			try {
				...
				logger.debug(
					`[ResourceManager] 清理Promise [${this.editorId}]: ${resourceId} (${
						promiseResource.description || "unknown"
					})`
				);
			} catch (error) {
		...
		for (const [resourceId, component] of this.components) {
			try {
				component.cleanup();
				logger.debug(
					`[ResourceManager] 清理组件 [${this.editorId}]: ${resourceId} (${
