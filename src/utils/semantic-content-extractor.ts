...
export class SemanticContentExtractor {
	...
	private initializePatterns(): void {
		...
		this.questionPatterns = [
			...
			},
			{
				...
				patterns: [/(定义|概念|原理|特点|优缺点)/i, /(definition|concept|principle|feature)/i],
				weight: 0.8,
			},
		];
		...
		this.answerPatterns = [
			{
				...
				patterns: [/^(答案?[:：]|解答[:：]|回答[:：])/i, /^(Answer[:：]?|Solution[:：]?)/i],
				weight: 1.0,
			...
			},
			{
				...
				patterns: [/^(因为|由于|所以|因此)/i, /^(Because|Since|Therefore|Thus)/i],
				weight: 0.7,
			},
		];
	...
	private extractAnswer(
		...
		if (answerSegments.length === 0) {
			// 如果没有明确的答案段落，使用非问题段落
			const nonQuestionSegments = allSegments.filter(
				(s) => s.type !== "question" && s.type !== "separator" && s.type !== "metadata"
			);
	...
	private determineExtractionMethod(
		segments: ContentSegment[]
	): SemanticExtractionResult["method"] {
		const hasKeywordMatches = segments.some((s) =>
			s.features.some((f) => f.startsWith("keyword:"))
		);
		const hasPatternMatches = segments.some((s) =>
			s.features.some((f) => f.startsWith("pattern:"))
		);
		const hasContextMatches = segments.some((s) =>
			s.features.some((f) => f.startsWith("context:"))
		);
		const hasHeuristicMatches = segments.some((s) =>
			s.features.some((f) => f.startsWith("heuristic:"))
		);
	 ...
	 * 生成警告信息
	 */
	private generateWarnings(segments: ContentSegment[], question: string, answer: string): string[] {
