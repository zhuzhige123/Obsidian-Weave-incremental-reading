 ...
 * ℹ 历史说明：
 * - 旧版本支持父子牌组层级
 * - v2.0+ 已废弃层级结构，相关函数已标记 Compatibility note:
 */
 ...
 * 构建牌组列表（平级架构）
 *
 * Compatibility note: 平级架构下 children 始终为空数组
 * @param decks 牌组列表
 ...
 * 扁平化节点列表
 *
 * Compatibility note: 平级架构下直接返回输入列表
 * @param nodes 节点列表
 ...
 * 获取牌组的缩进级别
 *
 * Compatibility note: 平级架构下始终返回 0
 * @param _path 已废弃
 * @returns 0
 */
 ...
 * @returns 匹配的节点列表
 */
export function searchDecks(nodes: DeckHierarchyNode[], query: string): DeckHierarchyNode[] {
	if (!query.trim()) return nodes;

	const lowerQuery = query.toLowerCase();
	return nodes.filter((node) => node.deck.name.toLowerCase().includes(lowerQuery));
}

/**
 * Compatibility note: 平级架构无展开/折叠概念，直接返回输入列表
 */
...
/**
 * Compatibility note: 平级架构无展开概念，返回所有牌组ID
 */
 ...
 * @returns 统计信息列表
 */
export function buildDeckStats(decks: Deck[], deckCardCounts: Map<string, number>): DeckStats[] {
