...
import {
	type PreviewToggleCallback,
	obsidianPreviewToggleExtension,
} from "./previewToggleExtension";
...
export function createEditorShortcuts(config: EditorShortcutsConfig = {}): Extension[] {
	const { onTogglePreview, enablePreviewToggle = true, customShortcuts = [] } = config;
 ...
 * @returns CodeMirror 扩展数组
 */
export function createMarkdownEditorShortcuts(onTogglePreview: PreviewToggleCallback): Extension[] {
 ...
 * @returns CodeMirror 扩展数组
 */
export function createBasicEditorShortcuts(customShortcuts: Extension[] = []): Extension[] {
	return createEditorShortcuts({
