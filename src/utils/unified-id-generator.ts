...
export class UnifiedIDGenerator {
	...
	generateWithDetails(
		type: IDType,
		customConfig?: Partial<IDGenerationConfig>
	): IDGenerationResult {
		const config = { ...this.DEFAULT_CONFIGS[type], ...customConfig };
	 ...
	 * Compatibility note: 请使用 WeaveIDGenerator.generateCardUUID() 生成新格式UUID
	 */
	// Legacy UUID v4 generator retained only for compatibility reference.
	private generateLegacyUUID(): string {
