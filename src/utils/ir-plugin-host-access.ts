	...
	try {
		...
		return {
			...
			flowStretchPercent: clampFlowStretchPercent(
				ir.flowStretchPercent ?? defaults.flowStretchPercent ?? DEFAULT_FLOW_STRETCH_PERCENT
			),
			...
			dailyReadingPointStretchCap: computeReadingPointStretchCap(
				clampDailyReadingPointCap(ir.dailyReadingPointCap ?? defaults.dailyReadingPointCap),
				clampFlowStretchPercent(ir.flowStretchPercent ?? defaults.flowStretchPercent),
				ir.dailyReadingPointStretchCap ?? defaults.dailyReadingPointStretchCap
			),
			horizonSpreadDays: Math.max(
				5,
				Math.min(14, Math.round(Number(ir.horizonSpreadDays ?? defaults.horizonSpreadDays ?? 7)))
			),
			enableHorizonSmoothing: ir.enableHorizonSmoothing !== false,
			interleaveProfile: ir.interleaveProfile ?? defaults.interleaveProfile ?? "related-soft",
			maxTopicSharePercent: Math.max(
				40,
				Math.min(90, Math.round(Number(ir.maxTopicSharePercent ?? defaults.maxTopicSharePercent ?? 60)))
			),
