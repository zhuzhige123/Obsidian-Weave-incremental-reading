...
export function getRandomCongratulation(): CongratulationMessage {
	return CONGRATULATION_MESSAGES[Math.floor(Math.random() * CONGRATULATION_MESSAGES.length)];
}
...
export function getRandomQuote(): NeuroScienceQuote {
	return NEUROSCIENCE_QUOTES[Math.floor(Math.random() * NEUROSCIENCE_QUOTES.length)];
}
