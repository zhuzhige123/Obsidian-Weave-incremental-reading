<script lang="ts">
  ...
  import { clearDocumentBodyChrome, patchDocumentBodyChrome } from '../../utils/document-body-chrome';
  ...
  function handleMouseMove(event: MouseEvent) {
    ...
    if (!hasMoved && distance > dragThreshold) {
      ...
      // 禁用页面滚动和选择
      patchDocumentBodyChrome({ userSelect: 'none', overflow: 'hidden' });
    }
  ...
  function handleMouseUp() {
    ...
    if (isDragging) {
      ...
      // 恢复页面的正常行为
      clearDocumentBodyChrome(['userSelect', 'overflow']);
    }
  ...
  function handleTouchStart(event: TouchEvent) {
    ...
    longPressTimer = setTimeout(() => {
      ...
      // 禁用页面滚动
      patchDocumentBodyChrome({ overflow: 'hidden', touchAction: 'none' });
    }, LONG_PRESS_DURATION);
  ...
  function handleTouchEnd(event: TouchEvent) {
    ...
    // 恢复页面正常行为
    clearDocumentBodyChrome(['overflow', 'touchAction']);
  ...
  onDestroy(() => {
    ...
    clearDocumentBodyChrome(['overflow', 'userSelect', 'cursor', 'touchAction']);
  });
